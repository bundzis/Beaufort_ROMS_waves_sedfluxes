...
wave_data = xr.open_dataset('/projects/brun1463/ROMS/Kakak3_Alpine_2020/Scripts/Waves/ERA5_2020_wave_data.nc')  # UPDATE PATH
# TEMP
...
# OG
era5_waves = wave_data.sel(valid_time=slice('2020-07-01 09:00:00', '2020-11-03 09:00:00')) # this is 2019-07-01 01:00:00 - 2019-11-03 01:00:00 in AKDT
# TEMP
...
# era5 number of lats
era5_lat_len = len(era5_waves.latitude.values)
#print('era5_lat_len', era5_lat_len)

# era5 number of lons
era5_lon_len = len(era5_waves.longitude.values)
...
# OG
wave_frc = '/projects/brun1463/ROMS/Kakak3_Alpine_2020/Include/wave_forcing_file_kaktovik_shelf_era5_2020_data002_tempnanfix.nc'  # UPDATE PATH
# TEMP
...
# Input grid (era5)
ds_in_era5 = era5_waves.copy() 
#ds_in_era5['lon_360'] = ds_in_hycom.lon.values
ds_in_era5['lon'] = ds_in_era5.longitude.values
ds_in_era5['lat'] = ds_in_era5.latitude.values
...
ds_in_era5_mask = xr.where(~np.isnan(ds_in_era5['shww'][0,:,:].values), 1, 0) 
print('got through nan mask', flush=True)
ds_in_era5['mask'] = (('lat', 'lon'), ds_in_era5_mask)
...
regridder_era52rho = xe.Regridder(ds_in_era5, ds_out_rho, method="bilinear", extrap_method='inverse_dist') #extrap_method="nearest_s2d"
regridder_era52rho

# Save the weights - only need to do this once
fn_era52rho = regridder_era52rho.to_netcdf('regrid_era52rho_weights.nc')
#print(fn_era52rho)

# Now use the regridder/weights to regrid the significant wave height
#dr_era52rho_shww = era5_waves['shww'].copy() # significant hieght of wind waves
dr_era52rho_shww = era5_waves['swh'].copy() # significant hieght of combined wind waves and swell
dr_out_era52rho_shww = regridder_era52rho(dr_era52rho_shww) 
dr_out_era52rho_shww

# Now use the regridder/weights to regrid the mean wave direction
#dr_era52rho_mdww = era5_waves['mdww'].copy() # mean direction of wind waves
dr_era52rho_mdww = era5_waves['mwd'].copy() # mean wave direction 
dr_out_era52rho_mdww = regridder_era52rho(dr_era52rho_mdww) 
dr_out_era52rho_mdww

# Now use the regridder/weights to regrid the peak wave period
dr_era52rho_pp1d = era5_waves['pp1d'].copy() # peak wave period 
dr_out_era52rho_pp1d = regridder_era52rho(dr_era52rho_pp1d) 
dr_out_era52rho_pp1d 
...
dr_out_era52rho_shww_nonan = np.empty((time_len, eta_rho_len, xi_rho_len))

# Mean wave direction
dr_out_era52rho_mdww_nonan = np.empty((time_len, eta_rho_len, xi_rho_len)) 

# Peak wave period 
dr_out_era52rho_pp1d_nonan = np.empty((time_len, eta_rho_len, xi_rho_len))
#print('got here 2')

# Make a copy of the OG array to work with
#print('got here 3')
# Significant wave height
dr_out_era52rho_shww_cp1 = dr_out_era52rho_shww.copy() 

# Mean wave direction
dr_out_era52rho_mdww_cp1 = dr_out_era52rho_mdww.copy() 

# Peak wave period 
dr_out_era52rho_pp1d_cp1 = dr_out_era52rho_pp1d.copy() 
#print('got here 4')
...
for t in range(time_len):
    ...
    field1 = dr_out_era52rho_shww_cp1[t,:,:]
    
    # Mean wave direction 
    field2 = dr_out_era52rho_mdww_cp1[t,:,:]
    
    # Peak wave period
    field3 = dr_out_era52rho_pp1d_cp1[t,:,:]


    # Use the Laplace Filter to get rid of nans
    #field = laplacefilter(field, 1000, toxi, toeta)
    field1 = laplacefilter(field1, 1000, toxi, toeta) # shww
    field2 = laplacefilter(field2, 1000, toxi, toeta) # mdww
    field3 = laplacefilter(field3, 1000, toxi, toeta) # pp1d
    

    # Multiply by the rho mask 
    #field = field * ds_out_rho.mask.values
    field1 = field1 * ds_out_rho.mask.values # shww
    field2 = field2 * ds_out_rho.mask.values # mdww
    field3 = field3 * ds_out_rho.mask.values # pp1d
    ...
    dr_out_era52rho_shww_nonan[t,:,:] = field1 # shww
    dr_out_era52rho_mdww_nonan[t,:,:] = field2 # mdww
    dr_out_era52rho_pp1d_nonan[t,:,:] = field3 # pp1d
...
# shww
# Make a dataset
dr_out_era52rho_shww_nonan_xr = xr.Dataset(
    data_vars=dict(shww_nonan=(['time','eta_rho', 'xi_rho'], dr_out_era52rho_shww_nonan)
                   ), 
    coords=dict(
        time=('time', time_akdt), 
...
dr_out_era52rho_shww_nonan2 = dr_out_era52rho_shww_nonan_xr.shww_nonan.interpolate_na(dim='time', method='linear', max_gap=None)
dr_out_era52rho_shww_nonan3 = dr_out_era52rho_shww_nonan2.fillna(0.0) 

# dir
# Make a dataset
dr_out_era52rho_mdww_nonan_xr = xr.Dataset(
    data_vars=dict(mdww_nonan=(['time','eta_rho', 'xi_rho'], dr_out_era52rho_mdww_nonan)
                   ), 
    coords=dict(
        time=('time', time_akdt), 
        eta_rho=('eta_rho', grid.eta_rho.values), 
...
# Inteprolate over the nans
dr_out_era52rho_mdww_nonan2 = dr_out_era52rho_mdww_nonan_xr.mdww_nonan.interpolate_na(dim='time', method='linear', max_gap=None)
dr_out_era52rho_mdww_nonan3 = dr_out_era52rho_mdww_nonan2.fillna(0.0)

# pwavet
# Make a dataset
dr_out_era52rho_pp1d_nonan_xr = xr.Dataset(
    data_vars=dict(pp1d_nonan=(['time','eta_rho', 'xi_rho'], dr_out_era52rho_pp1d_nonan)
                   ), 
    coords=dict(
        time=('time', time_akdt), 
        eta_rho=('eta_rho', grid.eta_rho.values), 
...
# Inteprolate over the nans
dr_out_era52rho_pp1d_nonan2 = dr_out_era52rho_pp1d_nonan_xr.pp1d_nonan.interpolate_na(dim='time', method='linear', max_gap=None)
dr_out_era52rho_pp1d_nonan3 = dr_out_era52rho_pp1d_nonan2.fillna(0.0)
# END TEMP 
...
# swh_interp_g[:,:,:] = dr_out_era52rho_shww_nonan[:,:,:] # shww
# dir_interp_g[:,:,:] = dr_out_era52rho_mdww_nonan[:,:,:] # mdww
# pdir_interp_g[:,:,:] = dr_out_era52rho_mdww_nonan[:,:,:] # pdww
# pwavet_interp_g[:,:,:] = dr_out_era52rho_pp1d_nonan[:,:,:] # pp1d
# TEMP FIX
swh_interp_g[:,:,:] = dr_out_era52rho_shww_nonan3[:,:,:] # shww
dir_interp_g[:,:,:] = dr_out_era52rho_mdww_nonan3[:,:,:] # mdww
pdir_interp_g[:,:,:] = dr_out_era52rho_mdww_nonan3[:,:,:] # pdww
pwavet_interp_g[:,:,:] = dr_out_era52rho_pp1d_nonan3[:,:,:] # pp1d
...
for tt in range(time_len):
    ...
    for ee in range(eta_rho_len):
        ...
        # Significant wave height 
        hs_tmp = dr_out_era52rho_shww_nonan[tt,ee,:]
        #print('hs_tmp: ', hs_tmp)
        # Peak wave period 
        tp_tmp = dr_out_era52rho_pp1d_nonan[tt,ee,:]
        #print('tp_tmp: ', tp_tmp)
        # Water depth
